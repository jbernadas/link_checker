...
def getRobotParser(loader, startUrl):
    ...
    rp = robotparser.RobotFileParser(page)
    ...
    def handle_starttag(self, tag, attrs):
        ...
        if tag.upper() == "A":
            ...
            if self.hasBlockedExtension(url) or self.redirects.count(url) > 0:
                return
            if self.robotParser is not None and not self.robotParser.can_fetch("sitemap_gen", url):
        ...
        if page is None:
            ...
            if urllib.parse.urlsplit(newUrl)[1] == server and \
               ...
               (robotParser is None or \
                robotParser.can_fetch("sitemap_gen", newUrl)):
